    ...
    <div className="mm-detail-backdrop" onClick={onClose}>
      <div className="mm-detail-card" onClick={e => e.stopPropagation()}>
        ...
        <div className="mm-detail-meta">
          <div><strong>{capture.store_code}</strong> — {capture.fixture_type}{capture.fixture_label ? ` (${capture.fixture_label})` : ''}{capture.bays ? ` · ${capture.bays} bay${capture.bays === 1 ? '' : 's'}` : ''}</div>
          ...
          {captures.map(c => (
            <div key={c.capture_id} className="mm-capture-card card" onClick={() => setSelected(c)}>
              ...
              <div className="mm-capture-card-body">
                <div className="mm-capture-card-title">{c.store_code} · {c.fixture_type}{c.fixture_label ? ` (${c.fixture_label})` : ''}{c.bays ? ` · ${c.bays} bay${c.bays === 1 ? '' : 's'}` : ''}</div>
